...
##
## Modified by "Naser Hossein Motlagh" (Aalto UNiversity, DIEM, Holmstrom Research Group, Built-by-Data project) 
...
#--------------- Write the Data in .cvs file --------------------------

    def report(self,tag,value=None):
        
#>>>>>>>START OF FILE WRITER DEFINITION<<<<<<<<<

	# File writer definition to automatically check for new devices in the csv sheet and write the data to respective file.

        def file_writer():
            read_data = open("sensor.csv") #Update the file called "sensor.csv" with new sensor mac addresses. ** Please put MAC Adresses in small lettters. <NO CAPS>** **Make sure the file saved is in UTF-8 encoding.** ** While selecting "Save As" in the final dialouge box select "As Text csv"**
            sensor_list = csv.reader(read_data, delimiter=",")

            print sensor_list
            f_count = 3 #Used for naming sensors. All the sensors starting from 4 are written and updated by this section. Once you comment/delete the old file writing code, this can be set to zero. Then  all the new sensor files will be written from 1, e.g Sensor_1.csv 
            for item in sensor_list:
                f_count = f_count+1
                print item[0]
                print str(self.addr)
                if str(self.addr) == str(item[0]):
                    print "entered loop"
                    fb = open('/home/pi/Data/Sensor_'+str(f_count)+'.csv','a+')
                    fb.write(str(now)+","+str(lux)+"\n")
                    fb.close()
...
#>>>>>>> CALL FOR THE FILE  WRITER DEFINITION <<<<<<<<< 

            file_writer() #since we initialized the parameters above, we dont need to pass any arguments. 


#-------------SensorTag: '98:07:2d:40:f1:04'  --------------------------------------------------------------
            if self.addr == '54:6c:0e:4d:6d:87':
                print '{"deviceuid":"'+self.addr+'"," devicename":"'+self.friendlyname+'"," '+tag+'":'+str(value)+'}'
               
                fb = open('/home/pi/Data/Sensor_1.csv','a+')
                fb.write(str(now)+","+str(lux)+"\n")
                fb.close()
                

#-------------SensorTag: '54:6c:0e:53:04:44' --------------------------------------------------------------
            if self.addr == '24:71:89:cf:bb:06':
#               print '{"deviceuid":"'+self.addr+'"," devicename":"'+self.friendlyname+'"," '+tag+'":'+str(value)+'}'

                fb = open('/home/pi/Data/Sensor_2.csv','a+')
                fb.write(str(now)+","+str(lux))
                fb.close()

#------------------SensorTag:  '98:07:2d:28:14:80' ------------------------------------------------------------
#            if self.addr == '98:07:2d:28:14:80':
#              print '{"deviceuid":"'+self.addr+'"," devicename":"'+self.friendlyname+'"," '+tag+'":'+str(value)+'}'

#                fb = open('/home/pi/Data/Sensor_3.csv','a+')
#                fb.write(str(now)+","+str(lux))
#                fb.close()
...
#-------------SensorTag: '98:07:2d:40:f1:04'  --------------------------------------------------------------
#                if self.addr == '98:07:2d:40:f1:04':
#                   fb = open('/home/pi/Data/Sensor_1a.csv','a+')
#                   fb.write("," +str(now)+ ","+str(lux)+'\n')
#                   fb.close()

#-------------SensorTag: '54:6c:0e:53:04:44' --------------------------------------------------------------
#                if self.addr == '54:6c:0e:53:04:44':
#                   fb = open('/home/pi/Data/Sensor_2a.csv','a+')
#                   fb.write("," +str(now)+ ","+str(lux)+'\n')
#                   fb.close()

#------------------SensorTag:  '98:07:2d:28:14:80' ------------------------------------------------------------
#                if self.addr == '98:07:2d:28:14:80':
#                   fb = open('/home/pi/Data/Sensor_3a.csv','a+')
#                   fb.write("," +str(now)+ ","+str(lux)+'\n')
#                   fb.close()
...
if not args.fast and not args.medium and not args.slow:
    ...
    args.medium = None
#    args.slow = ['barometer']
    args.slow = ['lightmeter','humidity']
    args.fast = None
#    args.fast = ['lightmeter','humidity','accelerometer','barometer','gyroscope','magnetometer']
